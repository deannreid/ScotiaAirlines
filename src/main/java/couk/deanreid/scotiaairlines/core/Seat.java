...
public class Seat {
    ...
    public Seat (int column, char row) {
        seatNumber = String.valueOf (column)
                + row;
        seatPrice = 100;
        seatTakings = 0f;
        currentStatus = 1;
    ...
    public int changeSeatStatus (int seatStatus, float currentTakings, String passengerName, char passengerType, String passengerInfo) {
        ...
        switch (passengerType) {
            ...
            case 'O': {
                PassengerOrdinary newPassenger = new PassengerOrdinary (passengerName, passengerInfo.
                        charAt (0));
                aPassenger = newPassenger;
                if (seatStatus
                        == 2) {
    ...
    /**
     * Method which changes status of particular seat depending on booking
     * choice made by passenger
     *
     * @param scotiaAirline
     * @param newStatus
     * @param newPassenger
            ...
            case 1:
                switch (currentStatus) {
                    ...
                    case 3:
                        ...
                        UI.genericPopup ("Seat Number: '"
                                + seatNumber
                                + "' Has Been Cancelled - No Refund Eligible");
                        break;
                    default:
                        break;
                }
            ...
            case 3:
                switch (currentStatus) {
                    case 1:
                        ...
                        aPassenger = newPassenger;
                        completeSeatStatus = 4;
                        seatTakings += (seatPrice
                                * newPassenger.getDiscountAmount ());
                         {
                            try {
                                NotificationHandler.
                                        notify ("Seat Booked", "Seat Number: '"
                                                + seatNumber
                                                + "' Has Now Been Booked By '"
                                                + newPassenger.
                                                        getPassengerName ()
                                                + "'");
                                LogHelper.debug ("Seat Number: '"
                                        + seatNumber
                                        + "' Has Now Been Booked By '"
                                        + newPassenger.getPassengerName ()
                                        + "'");
                        ...
                        break;
                    case 2:
                        if (newPassenger.getPassengerName ().
                                equalsIgnoreCase (aPassenger.getPassengerName ())) {
                            currentStatus = 3;
                            aPassenger = newPassenger;
                            ...
                            try {
                                NotificationHandler.
                                        notify ("Seat Already Reserved", "Seat Number: '"
                                                + seatNumber
                                                + "' Is Already Reserved By '"
                                                + aPassenger.getPassengerName ()
                                                + "'");
                    ...
                    case 3: {
                        try {
                            NotificationHandler.
                                    notify ("Seat Already Booked", "Seat Number: '"
                                            + seatNumber
                                            + "' Is Already Booked By '"
                                            + aPassenger.getPassengerName ()
                                            + "'");
