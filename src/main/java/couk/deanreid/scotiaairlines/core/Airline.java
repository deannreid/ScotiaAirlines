...
public class Airline {
    ...
    public void loadFlightsFromDB () throws SQLException {
        try {
            ...
            LogHelper.debug (Reference.TextPaint.GREEN
                    + "======================="
            ...
            while (rs.next ()) {
                ...
                LogHelper.debug (Reference.TextPaint.BLUE
                        + "Flight: '"
                        + flightNo
            ...
            LogHelper.error (Reference.TextPaint.RED
                    + "Flights Failed to Load from Database"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
    ...
    public void loadSeatsFromDB () throws SQLException {
        try {
            ...
            LogHelper.error (Reference.TextPaint.RED
                    + "Seats Failed to Load from database"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
    ...
    public void loadPassengersFromDB (String flightNo, Seat passengerSeat, int status, float takings) {
        try {
            ...
            while (rs.next ()) {
                ...
                char passengerType = rs.getString (3).
                        charAt (0);
                String passengerInfo = rs.getString (4);
                String flightNumber = rs.getString (5);
                if (seatNo.equalsIgnoreCase (passengerSeat.getSeatNumber ())
                        && flightNumber.equalsIgnoreCase (flightNo)) {
                    Flight passengersFlight = flights.get (flightNumber);
                    int choice = passengerSeat.
                            changeSeatStatus (status, takings, passengerName, passengerType, passengerInfo);
            ...
            LogHelper.error (Reference.TextPaint.RED
                    + "Passengers Failed to Load from database"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
    ...
    public void emptyDB () {
        ...
        try {
            LogHelper.warn (Reference.TextPaint.BLUE
                    + "DATABASE DEBUG: Cleaning DB"
                    + Reference.TextPaint.RESET);
            Connection connection = DBProxy.getConnection ();
            ...
            if (rs
                    > 0) {
                LogHelper.warn (Reference.TextPaint.BLUE
                        + "SEAT DEBUG: Seat Tables Emptied"
                        + Reference.TextPaint.RESET);
            }
        } catch (SQLException e) {
            LogHelper.error (Reference.TextPaint.RED
                    + "SEAT ERROR: Seat table cannot be emptied"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
        try {
            ...
            if (rs
                    > 0) {
                LogHelper.warn (Reference.TextPaint.BLUE
                        + "PASSENGER DEBUG: Passengers Emptied"
                        + Reference.TextPaint.RESET);
            }
        } catch (SQLException e) {
            LogHelper.error (Reference.TextPaint.RED
                    + "PASSENGER ERROR: Passenger table cannot be emptied"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
    ...
    public void saveSeatsToDB () {
        try {
            //for each flight
            for (Map.Entry<String, Flight> currentFlight
                    : flights.entrySet ()) {
                //for each seat in current flight
                for (Map.Entry<String, Seat> currentSeat
                        : currentFlight.getValue ().
                                getSeats ().
                                entrySet ()) {
                    //get values for current seat
                    String seatNo = currentSeat.getValue ().
                            getSeatNumber ();
                    String seatTakings = String.
                            valueOf (currentSeat.getValue ().
                                    getSeatTakings ());
                    String seatStatus = String.valueOf (currentSeat.getValue ().
                            getCurrentStatus ());
                    String flightNo = currentFlight.getValue ().
                            getFlightNumber ();
                    LogHelper.
                            debug ("SEAT DEBUG: Connecting and Preparing Statement");
                    Connection connection = DBProxy.getConnection ();
                    Statement stmt = connection.createStatement ();
                    LogHelper.debug ("SEAT DEBUG: Execute Statement: "
                            + stmt);
                    int rs = stmt.
                            executeUpdate ("INSERT INTO Seat (SeatNo, Status, Takings, FlightID) VALUES('"
                                    + seatNo
                                    + "','"
                                    + seatStatus
                                    + "','"
...
//                        LogHelper.info("SEAT DEBUG:" + Reference.TextPaint.BLUE + "Seat: " + seatNo + " booked on flight: " + flightNo + " by passenger: " + currentSeat.getValue().getaPassenger().getPassengerName() + Reference.TextPaint.RESET);
            ...
            LogHelper.error (Reference.TextPaint.RED
                    + "SEAT ERROR: Seats failed to save to DB"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
        ...
        try {
            ...
            for (Map.Entry<String, Flight> currentFlight
                ...
                for (Map.Entry<String, Seat> currentSeat
                    ...
                    if (currentSeat.getValue ().
                            getaPassenger ()
                            != null) {
                        if (currentSeat.getValue ().
                                getaPassenger ().
                                getClass ().
                                isInstance (new PassengerBusiness ())) {
                            type = 'B';
                            PassengerBusiness bp = (PassengerBusiness) currentSeat.
                                    getValue ().
                                    getaPassenger ();
                            info = bp.getCompanyName ();
                        } else if (currentSeat.getValue ().
                                getaPassenger ().
                                getClass ().
                                isInstance (new PassengerWestern ())) {
                            type = 'I';
                            PassengerWestern ip = (PassengerWestern) currentSeat.
                                    getValue ().
                                    getaPassenger ();
                            info = ip.getIslandOfResidence ();
                                ...
                                getaPassenger ().
                                getClass ().
                                isInstance (new PassengerOrdinary ())) {
                            type = 'O';
                            PassengerOrdinary op = (PassengerOrdinary) currentSeat.
                                    getValue ().
                                    getaPassenger ();
                        ...
                        Connection connection = DBProxy.getConnection ();
                        Statement stmt = connection.createStatement ();
                        LogHelper.debug ("PASSENGER DEBUG: Execute Statement: "
                                + stmt);
                        int rs = stmt.
                                executeUpdate ("INSERT INTO Passenger(SeatNo, PassengerName, Type, Information, FlightID) VALUES('"
                                        + currentSeat.getValue ().
                                                getSeatNumber ()
                                        + "','"
                                        + currentSeat.getValue ().
                                                getaPassenger ().
                        ...
                        for (int i = 0;
                                ...
                                < rs;
                                i++) {
                            LogHelper.debug ("PASSENGER DEBUG:"
                                    + Reference.TextPaint.BLUE
                                    + "Passenger Saved: "
                                    + currentSeat.getValue ().
                                            getaPassenger ().
                                            getPassengerName ()
                                    + Reference.TextPaint.RESET);
                            LogHelper.info (Reference.TextPaint.BLUE
                                    + "Passenger Saved: "
                                    + currentSeat.getValue ().
                                            getaPassenger ().
                                            getPassengerName ()
            ...
            LogHelper.error (Reference.TextPaint.RED
                    + "PASSENGER ERROR: Passenger failed to save to DB"
                    + Reference.TextPaint.RESET);
            LogHelper.error (e);
        }
    }
    ...
    public Seat getSeat (String FlightNo, String SeatNo) {
        if (flights.containsKey (FlightNo)) {
            ...
            if (currentFlight.getSeats ().
                    containsKey (SeatNo)) {
                Seat foundSeat = currentFlight.getSeats ().
                        get (SeatNo);
                LogHelper.debug ("SEAT DEBUG:"
                        + Reference.TextPaint.BLUE
                        + "Seat Found: "
                        + foundSeat
                        + Reference.TextPaint.RESET);
