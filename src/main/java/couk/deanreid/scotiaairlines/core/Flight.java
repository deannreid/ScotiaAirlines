...
public class Flight {
    ...
    public void addFlightToDB () throws AWTException, MalformedURLException, ParseException {
        ...
        if (!shouldLeaveLoop) {
            try {
                ...
                stmt.
                        executeUpdate ("INSERT INTO Flight(FlightID, Departure, Arrival, Rows, Columns, Date) VALUES('"
                                + flightNumber
                                + "','"
                                + departure
                                + "','"
                ...
                shouldLeaveLoop = true; // This will exit to the main menu instead of crashing the program
                NotificationHandler.
                        notify ("Failed to add Flight", "Failed to add Flight, Please make sure you have correctly inserted all fields");
                LogHelper.error (Reference.TextPaint.RED
                        + "FLIGHT ERROR:"
                        + "Failed to add Flight, Please make sure you have correctly inserted all fields"
                        + Reference.TextPaint.RESET);
                LogHelper.error (e);
            } catch (NumberFormatException e) {
                shouldLeaveLoop = true; // This will exit to the main menu instead of crashing the program
                NotificationHandler.
                        notify ("Failed to add Flight", "Failed to add Flight, You can only use numbers for Rows and Columns");
                LogHelper.error (Reference.TextPaint.RED
                        + "FLIGHT ERROR:"
                        + Reference.TextPaint.RESET
                        + e);
    ...
    public void deleteFlightFromDB () throws AWTException, MalformedURLException {
        try {
            ...
            stmt.executeUpdate ("DELETE FROM Flight WHERE FlightID('"
                    + flightNumber
                    + "','");
            NotificationHandler.notify ("Flight Deleted", "Flight '"
                    + flightNumber
            ...
            NotificationHandler.
                    notify ("Failed to delete Flight", "Failed to delete Flight");
            LogHelper.error (Reference.TextPaint.RED
                    + "FLIGHT ERROR:"
    ...
    public float calculateTotalFlightTakings () {
        ...
        seats.entrySet ().
                forEach (( tempSeat ) -> {
                    totalFlightTakings += tempSeat.getValue ().
                            getSeatTakings ();
                });
        return totalFlightTakings;
    }
    ...
    public String displayFlightInfo () {
        ...
        output = "<html> Flight No: "
                ...
                + "<br /> Date: "
                + date
                + "<br /> Number Of Free Seats: "
                + freeSeats
                + " <br /> Number Of Reserved Seats: "
                + reservedSeats
                + "<br /> Number Of Booked Seats: "
                + bookedSeats
                + "<br /> Status Message: "
                + statusMessage
                + "<br /> Total Flight Takings: £"
                + totalFlightTakings;
        output = output
                + "</html>";
        return output;
    ...
    public boolean isValidSeatNumber (String seatNo) {
        ...
        letter = seatNo.substring (element
                + 1);
        if (letter.length ()
                == 1) {
